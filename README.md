...
Claude Code 를 열고 채팅에 한 줄 치면 됩니다. 자료는 어디에 있어도 되고, 경로만
적어 주면 됩니다.

```
~/Downloads/보고서.pdf 로 발표자료 만들어 줘
```

파일이 없어도 됩니다. 시작이 셋 중 하나일 뿐이고, 그 뒤는 같습니다.

| 시작 | 예 | 무슨 일이 |
|---|---|---|
| 파일 | `~/Downloads/보고서.pdf 로 발표자료 만들어 줘` | 파일을 읽어 자료로 씁니다 |
| 채팅에 쓴 내용 | 내용을 한 쪽 넘게 채팅에 적고 «이걸로 만들어 줘» | 적으신 글이 그대로 자료입니다. 파일로 저장할 필요 없습니다 |
| 주제만 | `우리 회사 신제품 소개 자료 만들어 줘` | 범위와 깊이를 한 번 묻고, 웹에서 자료를 모아 문서로 만든 뒤 시작합니다. 숫자는 찾은 출처에 있는 것만 씁니다 |

워드 기획서도 같이 받으려면 «둘 다» 라고 덧붙이거나 인터뷰 화면에서 고르면 됩니다.
그다음은 브라우저가 알아서 열리고(`http://127.0.0.1:5050`), 누르는 자리는 네 번입니다.

| 화면 | 하는 것 |
|---|---|
| 인터뷰 | 미리 채워진 답 중 «제 짐작이에요» 만 보고 고치고, ①(언제 · 누가)만 직접 → «기획서 쓰기» |
| 기획서 | 읽고, «확인 필요» 가 있으면 채우기 → «뼈대로» |
| 뼈대 | 흐름 고르기, 장 순서 · 제목 손보기 → «디자인으로» |
| 디자인 | 색 · 분위기 · 사진 · 시안 고르기 → «이대로 만들기» |

그 뒤는 채팅이 혼자 합니다 — 사진, 장 그리기, 검사, 내보내기. 결과는
`projects/<날짜_이름>/exports/` 에 `…_기획서.docx` 와 `…_ver1.pptx` 로 남습니다.

**어느 폴더에서든 됩니다.** `python3 tools-local/sync_global_stubs.py` 를 한 번
돌려 두면 이 저장소 밖 어느 폴더에서 Claude Code 를 열어도 스킬을 찾고, 결과는
**그 폴더의** `projects/` 에 생깁니다. 이 저장소 안에서 열면 여기 `projects/` 에
생깁니다. 자료를 특정 폴더에 둘 필요는 없습니다.

채팅을 닫으면 멈춥니다. 화면 위에 «채팅이 기다리고 있어요» 가 떠 있으면 살아 있는
것입니다. 화면 위 번호(자료 · 인터뷰 · 기획서 …)를 누르면 지난 단계로 돌아가 볼 수
있습니다.
