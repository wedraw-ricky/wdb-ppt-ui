...
| 2026-09-05 | **글자 위계 여섯 단** — 30 / 21 / 17 / 16 / 14 / 12 | 화면 글자 138곳 중 129곳이 10~15px 안에 몰려 있었다. 크기 차이가 5px 안이면 위계가 아니라 그냥 다 작은 글씨다 — 무엇이 중요한지 눈이 못 고른다. 크기만이 아니라 굵기로도 벌린다 (`t-page`·`t-sect`·`t-card`·`t-body`·`t-sub`·`t-label`). 본문 16px 는 한글을 오래 읽어도 눈이 안 아픈 하한 |
