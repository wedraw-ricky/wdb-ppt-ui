...
- **무엇:** 자료 넣기부터 워드 문서와 발표자료 완성까지, 서비스 전체 화면의
  기준. 2026-09-16 전까지는 확인 화면 하나만 다뤘다. 화면 하나만 고치면 다른
  화면과 다른 서비스로 보였기 때문에 범위를 전체로 넓혔다.
- **누가 읽나:** 대표, 그리고 화면을 짓는 사람. 그래서 한국어로 적는다.
- **그림은 어디에:** 결정의 그림은 캔버스 둘에 있다.
  와이어프레임 <https://claude.ai/artifact/7QgWKcWnbyV93pYDWuEBpH> (18장),
  하이파이 <https://claude.ai/artifact/5mGpR5DEJQPuHduurZpomg> (17장).
  캔버스와 이 문서가 다르면 **이 문서가 결정**이고 캔버스는 그림이다.
- **코드는 이 문서대로다.** `ui/system/tokens.css` 가 아래 색·글자·간격 값이고,
  `ui/system/patterns.tsx` 와 `pick.tsx` 가 부품, `ui/src/shell.tsx` 가 구역
  일곱이다. 화면 아홉이 `ui/src/` 에 있고, 파이프라인이 아직 받쳐 주지 않는
  자리(자료 놓기, 걸린 것, 워드 내려받기, 인수인계 메모)는 화면이 «아직
  없어요» 라고 말한다. 주소 뒤 `#/1`~`#/8` 로 어느 화면이든 열어 볼 수 있다.

**기억할 한 줄:** *디자이너가 곁에서 대표와 이야기하며 만든다.* 메뉴를 읽고
바라는 도구가 아니라, 무엇을 왜 그렇게 잡았는지 먼저 말하고 결과물을 보여주는
사람이다.
...
| 띠 | 화면 | 결과물 |
|---|---|---|
| 기획 | 1 자료 넣기 → 2 인터뷰 → 3 기획서 | 기획서 (제목, 거버닝 메시지, 틀의 절) |
| 문서 | 4 보고서·기획서 | 워드 문서 (양식대로) |
| 발표 | 5 뼈대 → 6 디자인 → 7 만드는 중 → 8 완성 | 발표자료 (PPTX, PDF) |

인터뷰 ⑨ «무엇을 만들까요» 에서 갈린다. 보고서만이면 4에서 끝, 발표자료만이면
4를 건너뛰고, 둘 다면 마지막 장의 결론이 문서의 결론과 같아야 하고 뼈대에서
검사한다. 7이 멈추면 «제 쪽 문제예요» 화면이 7의 짝으로 뜬다.

분량은 네 단계다 (2026-09-16 결정, 아래 기록). 15장을 넘으면 파트가 단위라
목록과 스토리보드가 파트별 줄이 되고, 50장을 넘으면 편으로 한 번 더 묶어
편 사이를 점선으로 끊는다.

## 원칙 다섯

1. **디자이너가 곁에서 말해요.** 무엇을 왜 그렇게 잡았는지, 이유는 대표의
   상황으로 («경영진 회의실 프로젝터라서»). 본문 위가 아니라 오른쪽 곁의
   포스트잇이라 본문을 밀어내지 않는다.
2. **결과물이 제일 커요.** 고르면 결과물이 바뀐다. 고르기는 결과물 아래.
3. **답은 미리 채워요.** 자료에서 읽은 것은 채워 두고 «자료에서 확실», «제
   짐작이에요», «모르겠어요» 꼬리표를 붙인다. 확실은 안 봐도 되고, 짐작만
   보면 되고, 모름은 대표가 채운다. 자료에 없는 것(언제, 누가 쓰나)은
   짐작으로 채우지 않고 비워 두고 먼저 묻는다.
4. **지어내지 않아요.** 없는 숫자는 «확인 필요»로 비운다. 읽다가 걸린 것은
   걸렸다고 말한다.
5. **틀리면 제 쪽 문제라고 말해요.** 무엇이 남았는지부터. 기술 말은 없다.

디자이너가 아는 세 가지: 대표가 누구인지(지난번, 늘 하던 대로), 지금 상황
(마감, 발표자, 용도), 대표가 한 말(«대표님 말씀» 되비추기).

## 컨셉. 이 화면은 자기가 만드는 장표처럼 생겼다

2026-09-07 실측에서 요소 818개 중 브랜드색으로 칠해진 면이 0개였다. 대비·
크기 검사는 전부 통과했다. 무채색 화면이 검사를 제일 잘 통과하기 때문이다.
검사로 못 잡는 자리라서 규칙으로 적어 둔다.

**① 16:9 가 반복된다.** 고르는 것을 보여주는 자리는 예외 없이 16:9 장표다.
원본이 16:9 가 아닌 그림은 여백째로 앉힌다. 자르지 않는다.

**② 고른 색이 무대를 칠한다.** 결과물 판 안의 무대(장 축소판이 놓이는 어두운
면)가 30% 의 자리다. 발표자료에서 색을 고르면 무대와 그 위의 장이 그 색으로
다시 칠해진다. 아직 안 골랐으면 남색이 그 자리를 지킨다. **한 화면에 칠해진
면이 최소 하나.**

**③ 장표의 가구를 화면의 가구로 쓴다.** 무대 위의 장, 간지, 표지가 화면의
축소판이다. 새 장식을 만들지 않는다.

## 색. 60:30:10 은 팔레트가 아니라 **면적** 규칙

어떤 색을 쓰느냐가 아니라 화면의 몇 할을 덮어도 되느냐다. 색은 셋이고 예외가
하나다. 하이파이 캔버스 «화면 언어» 장이 이 표를 그림으로 보여준다.

| 몫 | 이름 | 값 | 어디 |
|---|---|---|---|
| 60 바탕 | 종이 · 흰 판 | `#F5F4EF` · `#FFFFFF` | 화면 바닥, 판, 카드 |
| 30 주색 | 남색 · 남색 물 | `#1E2B4A` · `#E9EDF5` | 무대, 고른 칩, 첫째 버튼, 대주제 번호, 편·부 헤더 / 메모 판, 짐작 꼬리표 |
| 10 강조 | 골드 · 골드 종이 | `#C9A24B` · `#FBF4E1` | 추천 꼬리표, 진행 막대, 되비추기, 지금 장 테 / 디자이너 포스트잇 |
| 글 | 잉크 · 보조 · 흐림 | `#1A1B1E` · `#5A5C63` · `#8A8C93` | 본문 / 부제·설명 / 근거·힌트 |
| 선 | 선 · 진한 선 | `#E4E2DA` · `#CBC8BE` | 판 경계 / 칩·카드·입력 경계 |
| 예외 | 주황 · 주황 물 | `#C2410C` · `#FFF3EC` | **«확인 필요» 꼬리표와 «걸린 것» 판에만** |

**규칙**
- 좋다는 뜻의 초록은 없다. 검사 통과는 ✓ 하나로 충분하다. 색 하나로 말하지
  않는다. 글이나 모양이 함께 간다 (WCAG 1.4.1).
- 골드는 «디자이너의 목소리»다. 추천, 되비추기, 포스트잇, 지금 장. 넓은 면을
  골드로 채우지 않는다.
- 남색 무대 위의 축소판은 남색 무대에서도 흰 띠에서도 보여야 한다. 그래서
  표지는 흰 바탕에 남색 판 62%, 간지는 남색에 흰 테 한 줄이다.
- 무대의 색은 변수 하나로 바뀐다. 고른 색이 무대를 칠하는 규칙은 코드에서도
  한 줄이어야 한다.
- 부품에 색을 손으로 칠하지 않는다. 토큰에서 물려받는다. `tokens.css` 는 이
  표로 바꾼다 (지금은 옛 indigo·blue·cyan 값).

## 글자. Pretendard, 다섯 단계

- **글꼴은 하나, Pretendard.** `CLAUDE.md` 의 글꼴 정책과 같다. 덱과 화면이
  같은 얼굴이어야 «이 화면이 저 덱을 만든다»가 보인다. 위계는 굵기와 크기로만.
- 다섯 단계뿐이다. 표 밖의 크기를 쓰지 않는다.

| 크기 | 굵기 | 어디 |
|---|---|---|
| 32 | 700 | 화면 제목. 질문이거나 결과 («이렇게 이해했어요») |
| 22 | 700 | 대주제. 번호가 붙는다 («① 언제, 누가 쓰나요») |
| 16 | 600 | 중주제 («어떤 느낌으로») |
| 15 | 400 | 본문, 디자이너 말, 칩·카드 이름(14) |
| 13 | 400 | 근거, 힌트, 판 이름표, 꼬리표(12) |

- 제목과 본문 사이에 두 단(22, 16)이 실제로 쓰인다. 32 에서 15 로 바로
  떨어지면 제목을 지난 뒤 전부 같은 목소리가 된다 (2026-09-07 실측).
- 말투는 해요체. 파란 메모(와이어프레임)만 명사형. 긴 줄표(—)는 쓰지 않는다.
  순서가 있으면 번호 목록, 없으면 점 목록으로 실제 목록 부품을 쓴다.
- `woff2` 로 `static/fonts/` 에 담아 둔다 (SIL OFL 1.1). `local()` 을 먼저
  적어 깔려 있으면 내려받지 않는다.

## 배치. 구역 일곱

기준 크기는 노트북 **1440×900**. 세로로 길어지는 것은 허용하고, 가로로
흐르는 것은 없다.

| 구역 | 크기 | 하는 일 |
|---|---|---|
| 머리띠 | 64 | 로고 · 기획/문서/발표 세 칸(지금 띠는 남색) · 지금 단계 한 줄 · 오른쪽 골드 진행 막대 260 |
| 제목 · 부제 | 32 · 16 | 본문 왼쪽 위. 질문이거나 결과 |
| 디자이너 포스트잇 | 곁 맨 위 | 골드 종이, 왼쪽 꼬리가 본문을 가리킨다. «디 · 디자이너 · 방금» |
| 결과물 판 | 본문, 제일 크게 | 흰 판 안에 남색 무대. 고르면 여기가 바뀐다 |
| 고르기 판 | 본문 | 번호 붙은 대주제 → 중주제 → 칩·카드. 접지 않는다 |
| 근거와 검사 판 | 곁 380 | 자료에서 읽은 것, 걸린 것(주황 물), 틀, 검사, 메모(남색 물) |
| 바닥 | 76 | 왼쪽 한 줄 안내 · 오른쪽 [둘째] [첫째]. 첫째 버튼은 화면에 하나 |

- 본문과 곁은 28 띄운 두 열이다. 곁은 380. 가장자리 48. 왼쪽 패널은 없다.
- 판 사이 28, 판 안 24, 부품 사이 12, 칩 사이 8. 라운드는 판 12 · 카드 8 ·
  칩과 버튼 6 · 꼬리표 4. 칩 36, 버튼 44.
- 대주제 안에서 디자이너가 짧게 말할 때는 그 대주제의 오른쪽 300 칸에 같은
  포스트잇으로 붙는다. 그 자리에서 하나만 더 물을 때도 같은 자리다.
- 긴 자료: 15장을 넘으면 결과물 무대가 파트별 한 줄, 첫 칸이 간지. 50장을
  넘으면 편 간지는 두 칸 폭, 편 사이 점선. 뼈대 목록은 편 헤더(남색) → 부 줄
  (회색, 작은 장 띠) → 연 부의 장만 펼친다.

## 부품

| 부품 | 모양 | 상태 |
|---|---|---|
| 칩 | 36, 라운드 6, 진한 선 | 고른 것 남색 채움 · 지금은 못 고름 점선 흐림 · 작은 칩 30 |
| 카드 | 라운드 8, 위에 16:9 축소판, 아래 이름 한 줄 | 고른 것 남색 테 2 · 추천은 골드 꼬리표 · 이름이 길면 말줄임 |
| 꼬리표 | 22, 라운드 4 | 추천(골드) · 확인 필요(주황) · 자료에서 확실(잉크 테) · 제 짐작이에요(남색 물) · 모르겠어요(주황 물) |
| 되비추기 | 골드 종이, «대표님 말씀 «…»» | 기획서 제목 아래, 뼈대 콜아웃 |
| 포스트잇 | 골드 종이, 왼쪽 꼬리 | 곁 맨 위(긴 말) · 대주제 오른쪽 300(짧은 말, 하나 더 묻기) |
| 판 | 흰, 라운드 12, 이름표 13 | 결과물(무대 포함) · 걸린 것(주황 물) · 메모(남색 물) |
| 무대 | 남색, 라운드 8, 안쪽 16 | 고른 색으로 다시 칠해짐. 편 사이 점선 |
| 장 축소판 | 16:9, 라운드 4 | 표지(흰+남색 62%) · 간지(남색+흰 테) · 본문(제목 띠+골드 줄) · 사진(남색 그러데이션) · 아직(점선 빈 칸) · 멈춤(주황 테) |
| 버튼 | 44, 라운드 6 | 첫째 남색 채움(화면에 하나) · 둘째 흰 · 멈추기는 테 없음 |
...
카탈로그를 통째로 넘기는 것은 고르라는 게 아니라 떠넘기는 것이다. 반대로
셋으로 줄이면 «선택의 폭이 너무 줄었다»가 된다 (2026-09-07, 대표). 그래서
순서와 무게로 푼다.

| | 디자이너가 하는 말 | 화면이 하는 일 |
|---|---|---|
| ① | «이렇게 잡았어요, 왜냐면 대표님 상황이» | 곁 맨 위 포스트잇. 이유는 대표 상황으로, 대표 말을 되비춘다 |
| ② | «여기까지 됐어요» | 결과물을 제일 크게. 고르면 여기가 바뀐다 |
| ③ | «답은 채워 뒀어요. 짐작만 봐 주세요» | 번호 대주제, 답 미리 채움, 확실·짐작·모름 꼬리표 |
| ④ | «하나만 더 여쭐게요» | 그 대주제 옆 포스트잇에서, 그 자리에서 |
| ⑤ | «지어낸 건 없어요. 걸린 건 이거예요» | 곁의 근거와 검사, 걸린 것은 주황 물 |
| ⑥ | «넘기기 전에 이것만» | 완성 화면의 인수인계 메모. 돌아가기 카드마다 남는 것과 바뀌는 것 |

**규칙**
- **접지 않는다.** 정해진 것도 보인다. 다만 결과에 미치는 영향 순서로
  놓는다. 색과 사진이 먼저, 규격과 제작 방식은 뒤. 접어 두고 «다 정하셨습니다»
  라고 말하는 것은 앞뒤가 안 맞는다.
- **후보는 한 줄에 넷, 추천이 첫째.** 나머지는 «더 보기». 열여덟을
  늘어놓지도, 셋으로 자르지도 않는다.
- **대표가 몰라도 되는 것은 묻지 않는다.** 결과가 같으면 물을 이유가 없다.
  알아서 정하고 바꿀 수 있게만 둔다. 틀은 인터뷰 ②에서 정해지고 다시 묻지 않는다.
- **익숙해지면 짧아진다.** 다섯 번째부터는 «늘 하던 대로 시작»이 먼저 나오고
  인터뷰가 짧아진다. 안내문도 짧아진다.
- **긴 자료는 나눠서 보여 준다.** 부(편) 하나가 끝날 때마다 보여주고 중간
  저장한다. 멈춰도 끝난 부는 남는다. 완성에서 고치기는 «이 부만 다시»가 기본.
...
| 2026-09-05 | **짜임은 넷뿐** — [`ui/system/patterns.tsx`](ui/system/patterns.tsx) | 토큰은 "무슨 값", 부품은 "무엇을 놓는가", 패턴은 "어떻게 짜는가". 셋이 다 있어야 다음 화면을 만들 때 판단할 게 없다. ①묻는 화면(`Shell`+`Ask`+`Pick`) ②다루는 화면(`Shell wide`+목록+서랍) ③알리는 화면(`Mid`) ④비었을 때(`Empty`) |
| 2026-09-05 | **빈 상태를 그냥 두지 않는다** | 목록이 비면 빈 격자가 나오거나 구역이 통째로 사라졌다 — 쓰는 사람은 **고장인지 원래 그런지 모른다.** 셋을 반드시 말한다: 무엇이 없는지 · 왜 없는지 · 무엇을 하면 되는지(할 수 있을 때만) |
| 2026-09-05 | 도달할 수 없는 빈 상태는 지운다 | "확인할 곳이 없습니다" 를 써 뒀는데, 0이면 그 버튼이 꺼져서 누를 수가 없었다. **죽은 코드는 없는 것보다 나쁘다** — 다음 사람이 그게 동작한다고 믿는다. 지우고 왜 안 두는지를 주석에 남겼다 |
| 2026-09-05 | **고르는 자리는 하나의 부품** — [`ui/system/pick.tsx`](ui/system/pick.tsx) | 고르는 부품이 **일곱 개**였다(템플릿·크기·서술 방식·아이콘·목록·그림 카드·이미지 출처). 하는 일은 같은데 — 격자에 카드를 놓고 하나 또는 여럿을 고른다 — 각각 따로 쓰여서 그림자·모서리·고른 표시가 제각각이었다. **화면마다 다르게 보이던 이유가 이것.** 다른 것은 *가운데에 무엇이 들어가느냐* 뿐이라 그 자리만 비워 뒀다 |
| 2026-09-05 | 부품마다 **어디 놓이는지**를 적는다 | 파란 패널용으로 만든 부품을 흰 화면에 옮겼더니 배경과 같은 색이 되어 사라진 적이 있다. 놓이는 면을 안 적어서 옮길 때 아무도 몰랐다. 이제 부품 머리말에 "놓이는 면" 이 있고 검사가 그것을 확인한다 |
| 2026-09-05 | 라디오 목록을 없애고 카드로 | 같은 앱에서 고르는 방식이 두 가지면 어떤 것이 눌리는지 매번 다시 배워야 한다. 글만 있는 질문도 시각 영역 없는 카드로 통일 |
| 2026-09-05 | **토큰을 세운다** — [`ui/system/tokens.css`](ui/system/tokens.css) | 카드가 두 종류, 모서리가 8·12·20px, 경계선이 1.15:1 과 3.37:1. 전부 같은 구멍의 증상이었다 — "다음 화면을 만들 때 뭘 재사용하는가" 에 답이 없었다. 여덟 갈래(원색 · 색 역할 · 글자 · 여백 · 모서리 · 선 굵기 · 그림자 · 누름 크기)를 한 파일에 두고, **값은 여기에만** 둔다. `theme.css` 는 이름만 잇는다 |
| 2026-09-05 | 이름은 "무엇인지" 가 아니라 "어디 쓰는지" | `--gray-300` 이면 어디 써야 할지 매번 판단해야 한다. `--line-strong` 이면 판단이 끝나 있다. 특히 선은 둘뿐이다 — `--line`(칸막이·장식)과 `--line-strong`(조작 요소 경계, 3:1). **이 둘을 섞어 쓴 것이 경계선 1.15:1 사고의 원인** |
| 2026-09-05 | 덱 팔레트 대체값은 토큰이 아니라 데이터 | 사용자가 아직 색을 안 골랐을 때 미리보기가 보여줄 값이라 화면 색 규칙과 다른 물건이다. 흩어져 있으면 화면 색으로 오해되므로 `DECK_FALLBACK` 한 곳에 모았다 |
| 2026-09-05 | 지켜지는지 기계가 본다 — [`tests/token-adherence.test.mts`](tests/token-adherence.test.mts) | 원티드 시스템에 `_adherence.oxlintrc.json` 이 있는 이유가 이것이다. 시스템을 만들고 끝이 아니라 지켜지는지 봐야 한다. 화면 코드에 색을 직접 쓴 곳, 토큰 갈래가 빠진 것, `theme.css` 가 값을 복사한 것을 잡는다 |
...
| 2026-09-05 | **네 구역을 한 장에** — «다음» 으로 넘기지 않는다 | 한 번에 한 구역만 보여주고 넘기면, 앞에서 무엇을 골랐는지 다시 보려면 뒤로 가야 하고 **고칠 것을 한꺼번에 말할 수 없다.** 계약(SKILL.md Step 4)도 원래 "한 번에 다 보여주고 한 번에 확정" 이 기본이고 단계별 확인은 사용자가 따로 요청할 때만이다 — 내가 기본값을 어기고 있었다 |
| 2026-09-05 | 맨 위 목록은 세는 자리가 아니라 **가는 자리** | 한 장에 펴 놓으면 "몇 번째 / 몇 개" 를 셀 것이 없어지고, 대신 8400px 짜리 화면에서 원하는 데로 바로 가는 일이 필요해진다. 아직 안 정한 것은 점과 색으로 함께 표시 (WCAG 1.4.1) |
...
| 2026-09-16 | 분량은 네 단계: 짧게 15장 미만 · 보통 15~30 · 길게 30~50 · 그 이상 50장 넘게. 15장을 넘으면 파트(부)가 단위가 되고 부마다 간지가 붙는다. 50장을 넘으면 부를 편으로 한 번 더 묶는다 | 화면의 분량 후보가 8~10 · 11~14 · 15~20 이었고 파이프라인 기본값(storyline.md §3)도 8~14 였다. 대표 기준으로는 그게 전부 «짧게» 다. 보통이 15~30 이면 파트가 예외가 아니라 기본이라, 뼈대·스토리보드·진행 화면은 파트별 배치가 기본형이다. 파이프라인의 틀별 기본 장 수는 화면을 만들 때 같이 손본다 |
| 2026-09-16 | 화면 기준을 확인 화면 하나에서 서비스 전체(기획·문서·발표 아홉 화면 + 긴 자료 일곱 화면)로 넓힌다. 결정의 그림은 캔버스 둘에 둔다 | 한 화면만 고치면 다른 화면과 다른 서비스로 보였다. 반복해서 «AI가 대충» 보인 이유가 기준 없는 땜질이었다. 흐름을 먼저 그리고 그 흐름만으로 화면을 그렸다 |
| 2026-09-16 | 색은 셋: 종이 60 · 남색 `#1E2B4A` 30 · 골드 `#C9A24B` 10. 주황은 «확인 필요»에만. 옛 indigo·blue·cyan 토큰은 폐기하고 `tokens.css` 를 이 표로 바꾼다 | 60:30:10 을 팔레트 나열로 보여 줬더니 «저 원칙 제대로 공부해봐». 면적으로 재서 62·28·10 이 되게 그렸고, 골드를 디자이너 목소리(추천·되비추기·포스트잇) 하나로 묶었다 |
| 2026-09-16 | 디자이너 말은 오른쪽 곁 맨 위의 포스트잇. 본문 위에 세로로 놓지 않는다. 대주제 안의 짧은 말도 그 대주제 오른쪽 300 칸 | «코멘트 느낌이 아니다. 포스트잇처럼 좌우로 들어가야 하는데 세로형이라 본문이 너무 내려간다». 옮기니 인터뷰가 2594 에서 2198 로 줄었다 |
| 2026-09-16 | 사람이 하는 것처럼: 지난번 기억, 언제·누가 먼저 묻기, 확실·짐작·모름 꼬리표, 읽다가 걸린 것, 대표님 말씀 되비추기, 이유는 대표 상황으로, 그 자리에서 하나만 더 묻기, 인수인계 메모, 남는 것과 바뀌는 것, «제 쪽 문제예요» 화면 | «AI가 해주는 거지만 사람이 작업해 주는 것 같은 디테일함과 공감, 이해가 UX에 포함되면 좋겠다». 열한 가지를 제안했고 «다 좋은 것 같은데 일단 그려보자» |
| 2026-09-16 | 자료에 없는 답(언제, 누가 쓰나)은 짐작으로 채우지 않고 비워 두고 먼저 묻는다 | «물어보고 채우는 게 낫겠지». 채워 둔 답에 «모르겠어요» 꼬리표를 붙이면 화면이 자기 말을 뒤집는 셈이다 |
| 2026-09-16 | 접지 않는다. 후보는 한 줄에 넷, 추천이 첫째, 나머지는 «더 보기». 2026-09-07 의 «후보 셋 · 정해진 것은 접기»를 바꾼다 | 셋으로 줄인 화면에 «선택의 폭이 너무 줄어든 느낌». 접힌 것은 «다 정하셨습니다»와 앞뒤가 안 맞았다 |
| 2026-09-16 | 카드 이름은 한 줄. 길면 줄인다 (스위스 · 안전 → 스위스). 넘치면 말줄임 | 추천 꼬리표가 붙은 첫 카드에서 이름이 두 줄로 접혔다. 열일곱 장 전부 돌려 두 줄인 곳이 없는 것을 확인했다 |
| 2026-09-16 | 화면은 파이프라인이 남긴 파일을 따라간다: 미리 채운 답(`draft`)이면 인터뷰, 뼈대 파일이 바뀌면 확정 전 화면이 다시 읽고, 기획서 화면도 뼈대가 오면 넘어간다. 서버는 디자인 후보가 없어도 기획 단계부터 뜬다 | 처음부터 끝까지 한 번 돌려 보니 인터뷰가 실제로는 열릴 수 없었고(답을 채워 두면 «답 끝난 것»으로 건너뜀, 후보가 없으면 서버가 안 뜸), 뼈대 화면이 빈 뼈대 8장을 먼저 집어 들었다 — «8장 밖에 안 나오는데?» |
| 2026-09-16 | 시안을 고르면 서술·분위기·글씨가 시안 기본값으로 바뀌고, 색과 글꼴은 시안이 정한다고 화면이 말한다. «시안 없이»로 돌아오면 추천값으로 되돌린다 | 시안 «apple»을 고른 채 남색이 칠해진 스토리보드를 보고 확정하셨다. 무채색 시안이라 남색이 사라지는 것을 채팅에서 여쭤야 했다 — 화면이 먼저 말했어야 한다 |
