...
> Keep external references in `svg_output/` during generation. Native PPTX export reads that source directly. When a self-contained SVG preview is requested, the on-demand `finalize_svg.py` step embeds images into `svg_final/`.
...
Use the unified pipeline in [`SKILL.md` Step 7](../SKILL.md). Native PPTX export reads `svg_output/` directly and converts it to native DrawingML. Run `finalize_svg.py` first only when a self-contained `svg_final/` preview is requested; the PPTX command does not consume `svg_final/` in the supported release route.

```bash
python3 scripts/finalize_svg.py <project_path>  # on demand only: self-contained SVG preview
