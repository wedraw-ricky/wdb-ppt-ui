...
|----------|----------|-------------|
| `IMAGE_BACKEND` | Optional | Backend identifier; defaults to `codex` (Codex CLI OAuth — needs `codex login`, no key). Run `image_gen.py --list-backends` for the current set |
| `IMAGE_CONCURRENCY` | Optional | Manifest-mode default concurrency (CLI `--concurrency` wins) |
| `{PROVIDER}_API_KEY` | Required for API backends only | Provider-specific API key, e.g. `GEMINI_API_KEY`, `ZHIPU_API_KEY`; the default `codex` backend needs none |
...
> Use provider-specific names only (e.g. `GEMINI_API_KEY`, `OPENAI_API_KEY`). In this install the default backend is `codex`, which reads no environment variables — a `.env` is only needed when explicitly switching to an API provider.
