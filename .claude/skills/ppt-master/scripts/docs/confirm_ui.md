...
The front-end loads `/api/catalogs` (served by the confirm server) and falls back to the static `/static/catalogs.json` if that route is unavailable. `/api/catalogs` returns the static file **with the `canvas` list synced live from `config.py CANVAS_FORMATS`** — the set of formats and their `dim` come from config (single source of truth, zero drift), while per-language labels / use text stay in catalogs.json (a plain fallback label is synthesized for any new id config adds). Keys: `canvas`, `modes`, `visual_styles` (grouped), `template_adherence`, `templates`, `icons`, `image_usage`, `image_ai_path`, `formula_policy`, `generation_mode`, `delivery_purpose`. Each entry is `{ "id", "label", "label_zh", "label_en", "label_ja", "label_ko", ... }`; descriptions use `desc_zh` / `desc_en` / `desc_ja` / `desc_ko`, and `visual_styles` groups use `group_zh` / `group_en` / `group_ja` / `group_ko`. The front-end falls back to legacy `label` / `desc` / `group`, so old catalogs still load, but new user-facing catalog text must cover all four languages (zh / en / ja / ko). English labels should mirror canonical reference names (`pyramid`, `swiss-minimal`, `Path A`, `mixed`, etc.); Chinese, Japanese, and Korean labels should be translated for users. Descriptions render inline after the option title, not as a separate selected-option line. `visual_styles` is `[{ "group", "group_zh", "group_en", "group_ja", "group_ko", "items": [...] }]`. For `canvas` you only need to maintain the per-language labels in catalogs.json; the format set and dimensions are authoritative in `config.py CANVAS_FORMATS`. `templates` is synced live from `templates/decks/decks_index.json` the same way: the static catalogs.json entry holds only the `free` option with its four-language labels; deck entries (`id`, `desc` from the index summary, `canvas_format`, `page_count`, `primary_color`, plus a `defaults` block whitelisted to the Stage-1 anchor ids `mode` / `visual_style` / `delivery_purpose` / `template_adherence`) are appended at serve time, so the card can never drift from the installed deck library.
...
> Each `candidates` array above shows **one** entry for brevity — `color` and `typography` must each carry **≥3** in a real file (a hard-locked field — install-local font lock, template-declared skin — is an honest-shortfall case and carries the single locked plan), while `image_strategy.candidates` should carry **exactly 3 non-custom recommendation** entries when AI image generation is offered. The UI adds the fourth **Custom** card itself; `selected` indexes the recommended default among the recommendation entries.

- `recommend.*` names the recommended `id` for each enumerable field (must match a `catalogs.json` id, or be a free string for a recommended custom value). The page badges and pre-selects it. **Guarantee**: if a `recommend.*` is omitted, the page falls back to the first catalog option so every enumerable field always shows one badged recommendation — but the AI should still set them for a meaningful default. Legacy aliases are accepted for old files (`line` → `tabler-outline`, `filled` → `tabler-filled`, `monochrome` → `chunk-filled`, `search` → `web`, `default` / `builtin` / `host-native` / `manual` → `auto`), but new files should write canonical ids.
- `recommend.template` names the Stage-1 template card pick: a deck id from `decks_index.json` only when the content genuinely matches that deck, otherwise `"free"` (the honest default). Omit when a template is already installed (locked card). When the confirmed stage-1 `result.json` carries a deck id, the AI runs the SKILL.md Step 3 deferred install (same structured preflight) before re-deriving Stage 2 — the template skin then **must** be authored as the recommended, pre-selected Stage-2 color / typography candidate. **Deck-pick cascade**: selecting a deck card re-defaults the Stage-1 anchors the deck declares — its `canvas_format` plus the index `defaults` ids (`mode` / `visual_style` / `delivery_purpose` / `template_adherence`) — overwriting the current value and moving the ★ badge; fields the deck does not declare stay untouched, and switching back to "free" restores the Strategist's original recommendations. The cascade fires only on a template change — every field remains freely editable afterwards.
...
- When `recommend.image_usage` includes `ai`, also set `recommend.image_ai_path` to one of `auto` / `codex` / `api`; the page presents these as explicit choices.
...
- **Candidate display text may be multilingual**: color / typography candidates can provide `name_zh` / `name_en` / `name_ja` / `name_ko` and `note_zh` / `note_en` / `note_ja` / `note_ko`; the page falls back to legacy `name` / `note`. Labels resolve in the page language first, then fall back across the others (a `ja` page: ja → en → zh → ko; a `ko` page: ko → en → zh → ja; zh/en pages keep their zh↔en fallback and try `_ja` / `_ko` last), so when `lang` is `ja` or `ko` always include the matching `_ja` / `_ko` variants — otherwise the candidate labels render in English.
- **Typography candidates split CJK and Latin** for both `heading` and `body`; `css` is the fallback preview `font-family` stack. The page previews CJK sample text with `cjk + css` and Latin sample text with `latin + css`, so the two script choices are visible independently. Each candidate should include topic-matched `sample_heading`, `sample_heading_latin`, `sample_body`, and `sample_body_latin`; do not reuse unrelated fixed examples such as a digital-transformation headline for a travel, education, product, or brand deck. Each candidate should also include `body_size` — the body baseline in **px** (the system's only unit, every canvas). The initial value comes from the candidate's `body_size`, sized for the recommended delivery purpose: `text` ~20 · `balanced` ~24 · `presentation` ~32. On submit the page writes `typography.body_size` as px directly — no pt conversion, no `body_size_pt` provenance. The page exposes `body_size` as an editable numeric field whose hint shows the recommended size — **one fixed px per delivery purpose on PPT (`text` 20 · `balanced` 24 · `presentation` 32), not a range** (non-PPT ≈2.5–3.3% of height in px). The input area labels the value as px, shows the SVG px → PPT pt relation (`1px = 0.75pt`) above the input, and displays the current approximate pt value below it for user orientation only; the output contract remains px-only. The user may still edit it; an out-of-range flag only warns if the value strays far (e.g. a unit mistake). **Canvas / delivery-purpose changes update only the hint — they never rewrite a value the user can see; editing `body_size` itself cascades the per-role sizes proportionally (next bullet).** It also offers a custom typography text box so the user is not limited to the proposed candidates.
- **Per-role size override** (parallel to color's per-role HEX override): besides `body_size`, the page exposes editable inputs for `title` / `subtitle` / `annotation`. Each role is pre-filled with a starting value — the candidate's `typography.sizes[role]` if provided, otherwise a ramp suggestion (`body × ` mid-band ratio). **Body-size cascade**: each role remembers its exact ratio-to-body (seeded from the ramp or the candidate value, and re-anchored whenever the user edits that role); editing `body_size` rescales all three roles from those ratios in one go (PPT even-px snap, 6–200 clamp), so user-edited roles keep their own proportion rather than reverting to the default ramp. `delivery_purpose` / canvas changes still update only the recommended-value hint, never the values; a re-render preserves exactly what the user sees, and every role stays freely editable after any cascade. Each role input is labelled as px and shows an approximate pt equivalent (`1px = 0.75pt`) for orientation. The final values are written to `result.json` as `typography.sizes: { "title", "subtitle", "annotation" }` in **px** — every canvas, no pt and no `sizes_pt` provenance. Seeding `sizes` in a candidate is optional — omit it and each role gets its ramp suggestion.
...
- `lang` is a soft default (`zh` / `en` / `ja` / `ko` — the page UI supports all four); an explicit user language choice in the page (persisted to `localStorage`) wins. With neither a stored choice nor a `lang`, this install's page defaults to Korean.
