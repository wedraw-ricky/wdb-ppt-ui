...
> Run when the user provides a raw `.pptx` template plus new content / a new topic and asks to generate a `.pptx` from that template. This skill treats the existing `.pptx` as a native slide library and produces a new `.pptx` by selecting, cloning, and patching source slides.
...
| User wants | Route |
|---|---|
| Generate a PPTX from a raw PPTX template | This skill |
| Directly edit / fill cloned PPTX slides | This skill |
| Create a reusable design asset from the PPTX | ppt-master [`create-template`](../ppt-master/workflows/create-template.md) |
...
python3 .claude/skills/ppt-master/scripts/project_manager.py import-sources "<project_dir>" "<source.pptx>" "<material...>"
```
...
| `<project_dir>/validation/` | Read-back Markdown, extracted validation assets, OfficeCLI scan output, rendered pages, and validation notes |
...
```bash
python3 .claude/skills/ppt-master/scripts/template_fill_pptx.py analyze "<project_dir>/sources/<source.pptx>" -o "<project_dir>/analysis/<stem>.slide_library.json"
```
...
```bash
python3 .claude/skills/ppt-master/scripts/template_fill_pptx.py scaffold "<project_dir>/analysis/<stem>.slide_library.json" -o "<project_dir>/analysis/fill_plan.json" --slides "1,3,4"
```
...
| Native tables | Keep the original table row and column count; edit ordinary cells or a merge anchor only. A merge slave is not visible and is rejected by both `check-plan` and `apply`. This skill never changes table structure. |
...
**Speaker notes (the `notes` field)** — distilled from the ppt-master main pipeline's Logic Construction Phase, scaled to one note per planned slide:
...
```bash
python3 .claude/skills/ppt-master/scripts/template_fill_pptx.py check-plan "<project_dir>/analysis/<stem>.slide_library.json" "<project_dir>/analysis/fill_plan.json" -o "<project_dir>/analysis/check_report.json"
```
...
> Note: `check-plan` is a pre-apply estimate. The post-apply OfficeCLI gate (Step 7.2) measures the rendered result; treat Step 5 as the cheap first filter, not the final word on fit.
...
```bash
python3 .claude/skills/ppt-master/scripts/template_fill_pptx.py apply "<project_dir>/sources/<source.pptx>" "<project_dir>/analysis/fill_plan.json" -o "<project_dir>/exports/<output.pptx>"
```
...
| Adds timestamp to PPTX filename | Matches the ppt-master SVG-to-PPTX export convention |
...
officecli view "<project_dir>/exports/<file>.pptx" issues --json
```

Save the `issues --json` output to `<project_dir>/validation/officecli_issues.json`.

**Triage `validate` results** (schema errors are triage signals, not automatic failures):

| Result | Action |
|---|---|
| No errors | Record pass, continue |
| Known-benign class (e.g. negative chart `axId` failing `UInt32` — python-pptx-lineage chart parts; PowerPoint tolerates it) | Record the exact error text as tolerated, continue |
| New / unclassified schema errors | Run the Step 7.3 render first; if content renders intact, record as tolerated with the exact error text; if parts fail to render, treat as an apply defect and fix the plan or report |

**Fix loop for `issues` findings** — repeat until the scan is clean or every remaining finding is recorded in `accepted_warnings`:

| Finding | Action |
|---|---|
| Text overflow on a filled frame (measured needed-vs-usable height) | Shorten the text in `fill_plan.json` for that slot — the report's suggested height indicates how much to cut. Re-run Step 5 `check-plan`, re-run Step 6 `apply`, re-scan. |
| Overflow on a frame this plan never touched (pre-existing in the source deck) | Not a fill defect. Record in `accepted_warnings` with `reason: "pre-existing in source template"`. |
| Overflow on an intentionally dense slot the user already approved | Record in `accepted_warnings` with the user's reason. |
| `broken_part_ref` / `notes_unresolved_rid` | Package defect from apply — fix the plan or re-run apply; if it persists, report the defect instead of shipping. |

**Hard rule**: never patch `<project_dir>/exports/*.pptx` in place. Exports are derived artifacts; every fix goes through `fill_plan.json` → `apply` → re-scan.

**Loop bound**: if the same finding survives 3 fix rounds, stop looping and surface it to the user with the measured numbers and the attempted rewrites — the slot may simply be too small for the required message, which is a page-selection decision (Step 3), not a wording problem.

**Missing OfficeCLI** → state the install command (`npm install -g @officecli/officecli@1.0.135`) and record in `<project_dir>/validation/` that the gate was skipped. Do not present the export as verified; get the user's explicit acceptance before treating it as final.
...
officecli view "<project_dir>/exports/<file>.pptx" screenshot --page 1-<N> \
  -o "<project_dir>/validation/pptx_render/deck.png"
# per page: --page <k> -o .../p<k>.png ; whole-deck contact sheet: --grid
```

| Check | Signal |
|---|---|
| Filled text fits its frame | No clipped lines or truncated paragraphs in filled slots |
| Unchanged pages look unchanged | Cloned pages must match the source deck design pixel-for-purpose |
| Fonts | Source deck fonts render; tofu boxes mean font substitution |
| Images / icons / SmartArt | No blank rectangles or missing assets on cloned slides |
| Tables / charts | Edited values render with the source formatting intact |

On a visual defect, route the fix through the same loop as 7.2: adjust `fill_plan.json`, re-apply, re-render. Under `--render html` (no Office installed) apply the known-deviation allowances from [`verify-pptx-export`](../ppt-master/workflows/verify-pptx-export.md) Step 3 — wrap points ±1 character, chart legend placement, and master date/slide-number placeholders are engine artifacts, not defects.

> Note: deeper post-export triage (package part inspection, SVG-route comparison) lives in ppt-master [`verify-pptx-export`](../ppt-master/workflows/verify-pptx-export.md); this skill's gate covers the fill-specific failure surface.
...
- [x] OfficeCLI `validate` triaged and `issues --json` clean (or every remaining finding recorded in `accepted_warnings`)
- [x] All pages rendered to `validation/pptx_render/` and visually checked; fixes routed through `fill_plan.json` → `apply`
```
...
| Measured visual overflow detection | Required — Step 7.2 OfficeCLI gate; falls back to Step 5 capacity judgment only when OfficeCLI is absent, and the export is then presented as unverified |
| Material-divergence reshaping (§c content strategy) | Not applicable — this skill fills text into existing slots, it does not author an outline from a source, so the ppt-master `content_divergence` free-text field has no role here |
